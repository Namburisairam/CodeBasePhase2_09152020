    ...
    public class AttendesLogic : BaseLogic
    {
        ...
        public async Task<bool> BookMarkActivity(int attendesID, int activityId)
        {
            ...
            else
            {
                try
                {
                    ...
                    bm.ActivityID = activityId;
                    bm.AttendesID = attendesID;
                    Db.BookMarks.Add(bm);
        ...
        public async Task<bool> AddUpdateAttendee(Attende attende, List<int> events, bool isUploadedImage)
        {
            if (attende.ID > 0)
            {
                ...
                if (attende.EnableMessaging == null)
                    updateAttendee.EnableMessaging = false;
                else
                    updateAttendee.EnableMessaging = attende.EnableMessaging;
                if (attende.EnableAlertEmails == null)
                    updateAttendee.EnableAlertEmails = false;
                else
                    updateAttendee.EnableAlertEmails = attende.EnableAlertEmails;
                if (attende.EnableAttendeeMessagingEmails == null)
                    updateAttendee.EnableAttendeeMessagingEmails = false;
                else
                    updateAttendee.EnableAttendeeMessagingEmails = attende.EnableAttendeeMessagingEmails;
                await Db.SaveChangesAsync();
                //update
            }
            else
        ...
        {
            checkAttendeeValidity = checkAttendeeValidity ?? new AtteendeeValidator();
            return await Db.Attendes.Where(checkAttendeeValidity.CheckValidity).Where(x => x.AttendesEvents.Any(y => y.EventID == EventId)).OrderBy(o => o.FirstName).ToListAsync();
        }

        public async Task<List<Attende>> GetAttendeeWithSpeakerForSpecificEvent(int EventId)
        {
            return await Db.Attendes.Where(x => x.AttendesEvents.Any(y => y.EventID == EventId) && x.IsAdmin == false).OrderBy(o => o.FirstName).ToListAsync();
        }
