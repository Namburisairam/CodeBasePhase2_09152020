    ...
    public class EventLogic : BaseLogic
    {
        ...
        public async Task<object> ActivitiesByEvent(int attendeeId, int eventId)
        {
            ...
            return attendeeEvent.Activites.Where(x => activities.Contains(x.ID)).Select(x => new
            {
                ...
                x.Thumbnail,
                x.ID,
                QRCode = x.QRCodes.Any() ? x.QRCodes.FirstOrDefault().WebURL : "",
        ...
        public HomePageData AssignHomeMetaData(HomePageData homePageData, Event @event, Func<string, string> imageConvertionHandler, int attendeeID, bool isAdmin)
        {
            ...
            homePageData.HomePageConfig = @event.WidgetMappings.OrderBy(x => x.YLocation).ThenBy(x => x.XLocation).Where(x => !(isAdmin && x.WidgetID == 4)).Select((x, idx) => new // removing the chat with staff if the logged in user is admin
            {
                x.DisplayName,
                x.ExternalLink,
                x.isExternalLink,
            ...
            //homePageData.RecentDiscussions = @event.ChatChannels.Select(x =>
            //x.TextChats.Where(y => y.SenderID == attendeeID || y.ReceiverID == attendeeID)
            //.OrderByDescending(y => y.Createdate)
            //.FirstOrDefault()).Where(x => x != null).Select(x => new
            //{
            ...
            //    Photo = x.IsGroup ? null : imageConvertionHandler(x.Attende.Thumbnail)
            //});
            homePageData.RecentDiscussions = @event.ChatChannels.Where(x => x.ChannelUsers.Any(y => y.AttendeeID == attendeeID))
              .Select(x =>
              x.TextChats
              .OrderByDescending(y => y.Createdate)
            ...
            homePageData.RecentFireChatConversations = UserChat;
            ...
            //homePageData.Attendees = @event.AttendesEvents.Where(x => x.EventID == @event.ID).Select(y => new 
            //{
            //    y.Attende?.ID,
            //    y.Attende?.FirstName,
            //    y.Attende?.Lastname,
            //    y.Attende?.Description,
            //    Thumbnail = imageConvertionHandler(y.Attende.Thumbnail),
            //    y.Attende?.FacebookURL,
            //    y.Attende?.TwitterURL,
            //    y.Attende?.InstagramURL,
            //    y.Attende?.Email,
            //    y.Attende?.LinkedinURL,
            //    y.Attende?.CompanyName
            //});

            //homePageData.Speakers = @event.AttendesEvents.Where(x => x.Attende.IsSpeaker == true).Select(y => new {
            //    y.Attende?.ID,
            //    y.Attende?.FirstName,
            //    y.Attende?.Lastname,
            //    y.Attende?.Description,
            ...
            //    y.Attende?.CompanyName
            //});
            //homePageData.Vendors = @event.VendorsEvents.Where(x => x.EventID == @event.ID).Select(y => new 
            //{
            //    y.Vendor?.ID,
            //    y.Vendor?.Name,
            //    Thumbnail = imageConvertionHandler(y.Vendor.Thumbnail),
            //    y.Vendor?.Description,
            //    PackageName= y.Vendor?.UserPackageType?.Name
            //});

            //homePageData.Surveys = ;
        ...
        {
            return await Db.Photos.Where(x => x.EventID == eventid && x.Status == false).ToListAsync();
        }
        ...
        public async Task<Event> GetEventById(int id)
        {
            if (id > 0)
            {
                ...
                var ss = events.FirstOrDefault(x => x.ID == id);
        ...
        public async Task<int> AddUpdateEvent1(Event saveEvent)
        {
            ...
            else
            {
                ...
                //Default Theme
                Theme th = new Theme();
                th.ButtonBackgroundColor = "#31a3dd";
                th.ButtonForegroundColor = "#ffffff";
                Db.Themes.Add(th);
                await Db.SaveChangesAsync();
                return saveEvent.ID;
            }
        ...
        public async Task<int> AddUpdateEvent(Event saveEvent)
        {
            ...
            else
            {
                ...
                th.ButtonBackgroundColor = "#31a3dd";
                th.ButtonForegroundColor = "#ffffff";
                th.LabelForegroundColor = "#000000";
                th.HeadingForegroundColor = "#000000";
                Db.Themes.Add(th);
                ...
                for (var i = 0; i < widgets.Count(); i++)
                {
                    ...
                    wM.XLocation = XLocation;
                    wM.YLocation = YLocation;
                    wM.WidgetID = widgets[i].ID;
                    wM.DisplayName = widgets[i].Name;
                    wM.widgetTypeID = widgets[i].defaultWidgetType;
                    wM.Width = 2;
                    wM.Height = 1;
                    saveEvent.WidgetMappings.Add(wM);
                }
                //var eventInfo = new eventinf();
        ...
        public async Task<bool> FetchActivities(int galEventid, int id, int eventID)
        {
            ...
            foreach (var value in result.ProgramActivities)
            {
                ...
                if (checkexistance == false)
                {
                    ...
                    ac.GalacticActivityId = value.ActivityID;
                    ac.StartTime = DateTime.Parse(value.ActivityDate);
                    ac.EndTime = DateTime.Parse(value.ActivityDate);
                    ac.Name = (value.ActivityName != null ? value.ActivityName : "");
                    ac.Address = "";
                    ac.Thumbnail = "";
                    ac.EventID = id;
                    ac.Status = true;
        ...
        public bool FetchEventInfo(int id)
        {
            ...
            if (result.EventInfos != null)
            {
                ...
                record.EventName = string.IsNullOrEmpty(result.EventInfos.Name) ? record.EventName : result.EventInfos.Name;
        ...
        //public async Task<IEnumerable<QuestionsWithOptions>> QuestionswithOptions(/*bool IsAtActvityLevel,*/ bool IsEventLevel, bool IsAtAppLevel, int UserID)
        //{
        ...
        //                                       (q, o) => new { q, o }).
        //                                       Where(x => x.q.IsAtEventLevel == IsEventLevel || x.q.IsAtAppLevel == IsAtAppLevel).
        //                                       Select(n => new QuestionsWithOptions
        //                                       {
        //                                           QID = n.q.ID,
        //                                           QuestionText = n.q.QuestionText,
        //                                           IsAtActiivtyLevel = n.q.IsAtActivityLevel,
        //                                           QuestionResponseType = n.q.QuestionResponseType,
        //                                           HideorShowQuestion = n.q.HideorShowQuestion,
        //                                           options = n.o.option1,
        //                                           IsSubmitted = false
        //                                       }).ToList();
        ...
        //    {
        //        var AttemptedQuestions = Db.SurveySubmittedFors.Where(x => x.UserID == UserID && (x.Question.IsAtEventLevel == true)).Count();
        //        if (AttemptedQuestions == 0)
        //        {
        //            var QuestionswithOptions = Db.Questions.
        //                                       Join(Db.Options, q => q.ID, o => o.QID,
        //                                       (q, o) => new { q, o }).
        //                                       Where(x => x.q.IsAtEventLevel == IsEventLevel).
        //                                       Select(n => new QuestionsWithOptions
        //                                       {
        //                                           QID = n.q.ID,
        //                                           QuestionText = n.q.QuestionText,
        ...
        //    else
        //    {
        //        var AttemptedQuestions = Db.SurveySubmittedFors.Where(x => x.UserID == UserID && (x.Question.IsAtAppLevel == true)).Count();
        //        if (AttemptedQuestions == 0)
        //        {
        //            var QuestionswithOptions = Db.Questions.
        //                                   Join(Db.Options, q => q.ID, o => o.QID,
        //                                   (q, o) => new { q, o }).
        //                                   Where(x => x.q.IsAtAppLevel == IsAtAppLevel).
        //                                   Select(n => new QuestionsWithOptions
        //                                   {
        //                                       QID = n.q.ID,
        //                                       QuestionText = n.q.QuestionText,
        ...
        //    var AttemptedQuestions = Db.SurveySubmittedFors.Where(x => x.UserID == UserID && (x.Question.IsAtActivityLevel == true && x.ActivityID == ActivityID)).Count();
        //    if (AttemptedQuestions == 0)
        //    {
        //        var QuestionswithOptions = Db.Questions.
        //                            Join(Db.Options, q => q.ID, o => o.QID,
        //                            (q, o) => new { q, o }).
        //                            Where(x => x.q.IsAtActivityLevel == true).
        //                            Select(n => new QuestionsWithOptions
        //                            {
        //                                QID = n.q.ID,
        //                                QuestionText = n.q.QuestionText,
        ...
        public async Task<bool> AddNewSurvey(Response[] SurveyReposne, int UserID, int? eventID = null, int? activityID = null)
        {
            try
            {
        ...
        public async Task sendPushNotificationWithEmail(string deviceToken, string subject, string body, int eventID, int aid, int sentBy, int notificationID)
        {
        ...
        public async Task<int> SendNotificationAndLogInDB(string deviceToken, string subject, string body, int eventID, int aid, int sentBy, int notificationID)
        {
            Notifications notifications = new Notifications();
        ...
        public async Task<bool> AddResyncedAttendees(GPSAttendees gPSAttendees, Func<ProgramAttendee, bool> validator, int eventID, string ImageSavePath)
        {
            ...
            foreach (var item in gPSAttendees.ProgramAttendees.Where(validator))
            {
                ...
                if (attendee == null)
                {
                    ...
                    at.Thumbnail = attendesLogic.GenerateImageFromName(item.FirstName, item.LastName, ImageSavePath);
                    at.Email = item.Email;
        ...
        private void CopyingAttendees(int sourceEventId, int targetEventId)
        {
            ...
            foreach (AttendesEvent attendee in attendees)
            {
                ...
                {
                    AttendesID = attendee.AttendesID, IsAdmin = attendee.IsAdmin, EventID = targetEventId
                };
