    ...
    public class SponsorsLogic : BaseLogic
    {
        ...
        public List<Sponsor> GetSponsorsForSpecificEvent(int EventId)
        {
            ...
            if (isSponsorNameSort && isSponsorTypeSort)
            {
                ...
                {
                    return Db.Sponsors.Where(x => x.SponsorsEvents.Any(y => y.EventID == EventId)).ToList().OrderBy(z => Helpers.HtmlToString(z.Name)).ThenBy(a => a.SponsorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
                else if (isSponsorNameAscendingSort && !isSponsorTypeAscendingSort)
                {
                    return Db.Sponsors.Where(x => x.SponsorsEvents.Any(y => y.EventID == EventId)).ToList().OrderBy(z => Helpers.HtmlToString(z.Name)).ThenByDescending(a => a.SponsorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
                else if (!isSponsorNameAscendingSort && isSponsorTypeAscendingSort)
                {
                    return Db.Sponsors.Where(x => x.SponsorsEvents.Any(y => y.EventID == EventId)).ToList().OrderByDescending(z => Helpers.HtmlToString(z.Name)).ThenBy(a => a.SponsorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
                else
                {
                    return Db.Sponsors.Where(x => x.SponsorsEvents.Any(y => y.EventID == EventId)).ToList().OrderByDescending(z => Helpers.HtmlToString(z.Name)).ThenByDescending(a => a.SponsorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
            }
            ...
            else if (isSponsorTypeSort)
            {
                ...
                {
                    return Db.Sponsors.Where(x => x.SponsorsEvents.Any(y => y.EventID == EventId)).ToList().OrderBy(z => z.SponsorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
                else
                {
                    return Db.Sponsors.Where(x => x.SponsorsEvents.Any(y => y.EventID == EventId)).ToList().OrderByDescending(z => z.SponsorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
            }
