    ...
    public  class VendorsLogic : BaseLogic
    {
        ...
        public async Task<List<Vendor>> GetVendorForSpecificEvent(int EventId)
        {
            ...
            if(isVendorNameSort && isVendorTypeSort)
            {
                ...
                {
                    return Db.Vendors.Where(x => x.VendorsEvents.Any(y => y.EventID == EventId)).ToList().OrderBy(z => Helpers.HtmlToString(z.Name)).ThenBy(a => a.VendorsEvents.FirstOrDefault(b => b.EventID== EventId)?.UserPackageType?.ID).ToList();
                }
                else if(isVendorNameAscendingSort && !isVendorTypeAscendingSort)
                {
                    return Db.Vendors.Where(x => x.VendorsEvents.Any(y => y.EventID == EventId)).ToList().OrderBy(z => Helpers.HtmlToString(z.Name)).ThenByDescending(a => a.VendorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
                else if(!isVendorNameAscendingSort && isVendorTypeAscendingSort)
                {
                    return Db.Vendors.Where(x => x.VendorsEvents.Any(y => y.EventID == EventId)).ToList().OrderByDescending(z => Helpers.HtmlToString(z.Name)).ThenBy(a => a.VendorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
                else
                {
                    return Db.Vendors.Where(x => x.VendorsEvents.Any(y => y.EventID == EventId)).ToList().OrderByDescending(z => Helpers.HtmlToString(z.Name)).ThenByDescending(a => a.VendorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
            }
            ...
            else if (isVendorTypeSort)
            {
                ...
                {
                    return Db.Vendors.Where(x => x.VendorsEvents.Any(y => y.EventID == EventId)).ToList().OrderBy(z => z.VendorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
                else
                {
                    return Db.Vendors.Where(x => x.VendorsEvents.Any(y => y.EventID == EventId)).ToList().OrderByDescending(z => z.VendorsEvents.FirstOrDefault(b => b.EventID == EventId)?.UserPackageType?.ID).ToList();
                }
            }
            else
            {
