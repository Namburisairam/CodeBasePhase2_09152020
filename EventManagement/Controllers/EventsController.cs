...
using static EventManagement.Models.FloorMapResponse;
...
namespace EventManagement.Controllers
{
    ...
    public class EventsController : BaseController
    {
        ...
        public async Task<ActionResult> Create(int? id)
        {
           ...
           // eventLogic.ResyncEventData(id.Value, ImageSavePath, CurrentUserSession.AttendesID.Value);
        ...
        public async Task<JsonResult> FetchAttendeeFromGalactic(int eventid, int mainid, bool isSpeakerRequest = false)
        {
            ...
            await eventLogic.FetchAttendeeFromGalactic(eventid, mainid, ImageSavePath, isSpeakerRequest);
            //AuthLogic al = new AuthLogic();
            //var records = al.GetProgramAttendees(eventid);
        ...
        {            
            await eventLogic.FetchUserGroupsFromGalactic(eventid, mainid, ImageSavePath, CurrentUserSession.AttendesID.Value);
        ...
        public async Task<ActionResult> PassValues(string jsond, int? fmid)
        {
            ...
            foreach (var item in values.Pin)
            {
                ...
                fm.FMid = fmid.Value;
                fm.X = Convert.ToDecimal(item.coords.lat) + 16; // 16 is for correction
                fm.Y = Convert.ToDecimal(item.coords.@long);// 16 is for correction
                fm.Description = item.content;
        ...
        private string ConvertListbasedJsonToNodebased(FloorMapResponse _flormap)
        {
            ...
            respodata.Add(canvaskey, _flormap.canvas);
            ...
            respo.Add(parentjsonkey, respodata);
        ...
        public async Task<ActionResult> CreateQrCode(FormCollection form)
        {
            if (form.Count > 0)
            {
                ...
                var qrCode = await eventLogic.EventQrCode(eventId, "", CurrentUserSession.AttendesID.Value, description, form["WebUrl"]);
        ...
        public async Task<ActionResult> ActivityQrCode(int id)
        {
            try
            {
                ...
                var codeid = await eventLogic.ActivityQrCode(id, "", CurrentUserSession.AttendesID.Value);
                var qCode = GenerateEventQrCode(id, "", enQRCodeFor.Activity, codeid);
        ...
        [HttpPost]
        public ActionResult ResyncEventData(int eventId)
        {
            eventLogic.ResyncEventData(eventId, ImageSavePath, CurrentUserSession.AttendesID.Value);
            return Json(true, JsonRequestBehavior.AllowGet);
        }
       }
}
